...
def update_til(change, til):
    # Doesn't look to see if there are enough quarters to cover when there
    # aren't enough dollars or nickels for dimes and so on
...
def print_change_and_til(change, til):
    if update_til(change, til):
        ...
        print(f"Expressed in the fewest amount of bills and coins, you have \n\
              {change[0]} dollars {change[1]} quarters, {change[2]} dimes,\n\
              {change[3]} nickels, and {change[4]} pennies.\n")
